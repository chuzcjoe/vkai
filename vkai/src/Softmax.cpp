...
Softmax::Softmax(core::vulkan::VulkanContext *context,
...
void Softmax::Run(VkCommandBuffer command_buffer) {
  ...
  const uint32_t group_x =
      (static_cast<uint32_t>(uniform_data_.batch_size) + kLocalSize - 1) /
      kLocalSize;
